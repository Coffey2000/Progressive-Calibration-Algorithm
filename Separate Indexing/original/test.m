global num_target_gain_states num_target_phase_states Measurements Mapping Current_Calibration_Gain_Index Current_Calibration_Phase_Index target_gain_states ...
    ...
    measurement_counter original_RTPS_gain_resolution num_actual_gain_states num_actual_phase_states gain_profile max_gain_measurement max_target_gain ...
     phase_offset target_gain_states_dB_normalized actual_phase_resolution actual_phase_states min_target_gain channel1_S21_38r5GHz_phaseCorrected total_measurement_counter kernel_offset max_gain PS_MODEL
...
while (next_state ~= "Finish Calibration")
    ...
    if num_next_measurements ~= 0
        ...
        if present_state == "Next Target Point"
            current_measured_points = zeros(num_next_measurements, 3);
    
            for i = 1:1:num_next_measurements
                next_gain = next_measurements(i, 1)/max_gain;
                next_phase = conversionClass.wrap22pi(next_measurements(i, 2))/(2*pi);

                model_output = PS_MODEL([next_gain; next_phase]);
                ...
                current_measured_points(i, 1) = measurementClass.measure([gain_index phase_index], "index");
            ...
            for i = 1:1:num_next_measurements
                ...
                if present_state == "Gain Profile Characterization"
                    plot(current_measured_points(i, 1)+0.001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "Color", [0 0.4470 0.7410]);
                    hold on
                    drawnow
                end
            end
...
global phase_offset num_target_phase_states Mapping Current_Calibration_Gain_Index Current_Calibration_Phase_Index target_gain_states ...
    ...
    target_gain_states_dB_normalized target_gain_states_dB min_target_gain actual_gain_resolution kernel_offset measurement_counter max_gain
