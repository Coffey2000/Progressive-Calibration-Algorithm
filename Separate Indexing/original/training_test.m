global total_measurement_counter

total_measurement_counter = 2000;

while total_measurement_counter >= 1100
clear
...
[PS_MODEL, tr] = train(PS_MODEL, input, output,'useGPU','no');
...
global num_target_gain_states num_target_phase_states Measurements Mapping Current_Calibration_Gain_Index Current_Calibration_Phase_Index target_gain_states ...
    ...
    measurement_counter original_RTPS_gain_resolution num_actual_gain_states num_actual_phase_states gain_profile max_gain_measurement max_target_gain ...
     phase_offset target_gain_states_dB_normalized actual_phase_resolution actual_phase_states min_target_gain channel1_S21_38r5GHz_phaseCorrected total_measurement_counter kernel_offset phase_kernel_size gain_kernel_size
...
phase_kernel_size = kernel_size;
gain_kernel_size = kernel_size;
...
plot(Selected_Measurements(Starting_Gain_Index:Current_Calibration_Gain_Index, :) + 0.0001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "MarkerFaceColor", "g");
hold on
...
global phase_offset num_target_phase_states Mapping Current_Calibration_Gain_Index Current_Calibration_Phase_Index target_gain_states ...
    ...
    target_gain_states_dB_normalized target_gain_states_dB min_target_gain actual_gain_resolution kernel_offset measurement_counter max_gain phase_kernel_size gain_kernel_size
...
switch present_state
    ...
    case "Gain Profile Characterization"
        ...
        plot(conversionClass.polar2cartesian(target_gain_states(Current_Calibration_Gain_Index), target_phase_states(Current_Calibration_Phase_Index)) + 0.0001*1i, 0, "O", "LineWidth", 1.5, "MarkerSize", 10, "MarkerFaceColor", [0 0.4470 0.7410]);
        hold on
        plot_gain_circle(target_gain_states(Current_Calibration_Gain_Index));
    ...
    case "Next Target Point"
        ...
        if Current_Calibration_Gain_Index > Starting_Gain_Index
            plot(Selected_Measurements(Starting_Gain_Index:Current_Calibration_Gain_Index - 1, :) + 0.0001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "MarkerFaceColor", "g");
            hold on
        end
        plot(Selected_Measurements(Current_Calibration_Gain_Index, 1:Current_Calibration_Phase_Index - 1) + 0.0001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "MarkerFaceColor", "g");
        xlim([-1*(target_gain_states(Current_Calibration_Gain_Index)+0.1) target_gain_states(Current_Calibration_Gain_Index)+0.1]);
        ylim([-1*(target_gain_states(Current_Calibration_Gain_Index)+0.1) target_gain_states(Current_Calibration_Gain_Index)+0.1]);
        drawnow
        hold on
        ...
        if valid
            ...
            phase_kernel_size = original_kernel_size;
            gain_kernel_size = original_kernel_size;
            % RTPS_phase_resolution = original_RTPS_phase_resolution;
            % RTPS_gain_resolution = original_RTPS_gain_resolution;

            plot(conversionClass.polar2cartesian(closest_measured_point(1, 2), closest_measured_point(1, 3)) + 0.0001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "MarkerFaceColor", "r");
            hold on
            ...
            if Current_Point_Iteration_Count > 8
                 phase_kernel_size = phase_kernel_size + 2;
                 gain_kernel_size = gain_kernel_size + 2;
...
global Current_Calibration_Gain_Index Current_Calibration_Phase_Index kernel_size actual_phase_resolution target_gain_states ...
    target_phase_states actual_gain_resolution min_target_gain max_target_gain phase_kernel_size gain_kernel_size

k = 0;
for gain = 1:1:gain_kernel_size
    for angle = 1:1:phase_kernel_size
        next_gain = target_gain_states(Current_Calibration_Gain_Index) + actual_gain_resolution * (gain - (gain_kernel_size + 1)/2);
        next_angle = conversionClass.wrap22pi(target_phase_states(Current_Calibration_Phase_Index) + actual_phase_resolution * (angle - (phase_kernel_size + 1)/2));
        
        if (next_gain >= min_target_gain/2) && (next_gain <= max_target_gain + 2*gain_kernel_size*actual_gain_resolution) && (next_angle >= 0) && (next_angle <= 2*pi)
            k = k + 1;
        end
    end
end
...
global Current_Calibration_Gain_Index Current_Calibration_Phase_Index kernel_size actual_phase_resolution target_gain_states ...
    target_phase_states actual_gain_resolution min_target_gain max_target_gain kernel_offset phase_kernel_size gain_kernel_size
    ...
    for gain = 1:1:gain_kernel_size
        for angle = 1:1:phase_kernel_size
            next_gain = new_target_gain + actual_gain_resolution * (gain - (gain_kernel_size + 1)/2);
            next_angle = conversionClass.wrap22pi(new_target_phase + actual_phase_resolution * (angle - (phase_kernel_size + 1)/2));
