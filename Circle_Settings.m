...
vector1_phase_distance = zeros(2^bit_length, 1);
vector2_phase_distance = zeros(2^bit_length, 1);

vector1_phase = zeros(2^bit_length, 1);
vector2_phase = zeros(2^bit_length, 1);

for i=1:1:2^bit_length
vector1_phase_distance(:, 1) = abs(phase - vector1_ideal_phase(i, 1));
index = find(vector1_phase_distance == min(vector1_phase_distance));
vector1_phase(i, 1) = phase(index);

vector2_phase_distance(:, 1) = abs(phase - vector2_ideal_phase(i, 1));
index = find(vector2_phase_distance == min(vector2_phase_distance));
vector2_phase(i, 1) = phase(index);
end
