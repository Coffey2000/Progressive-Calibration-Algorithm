...
FILE = fopen('simulation_data_mode3.txt');
...
simulation_data_mode3 = zeros(size(gain, 1), 1);

for j = 1:1:size(gain, 1)
    simulation_data_mode3(j, 1) = gain(j, 1) * cos(phase(j, 1)) + 1i * gain(j, 1) * sin(phase(j, 1));
end

save("simulation_data_mode3.mat", "simulation_data_mode3");
