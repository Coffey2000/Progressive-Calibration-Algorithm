...
while (next_state ~= "Finish Calibration")
    ...
    if num_next_measurements ~= 0
        ...
        for i = 1:1:num_next_measurements
            current_measured_points(i, 1) = measurementClass.measure(next_measurements(i, :), next_choice, 1);
...
global Current_Calibration_Gain_Index Current_Calibration_Phase_Index kernel_size RTPS_phase_resolution RTPS_gain_resolution_dB target_gain_states_dB target_gain_states...
    target_phase_states lowest_detectable_gain_dB lowest_detectable_gain RTPS_gain_resolution achievable_gain_states

    num_achievable_gain_states = size(achievable_gain_states, 2);
    target_gain = target_gain_states(Current_Calibration_Gain_Index);
    gain_distance = abs(achievable_gain_states(1, :) - target_gain);
    current_gain_index = find(gain_distance == min(gain_distance));
    
    if length(current_gain_index) > 1
        current_gain_index = current_gain_index(end);
    end
    ...
    for gain = 1:1:kernel_size
        for angle = 1:1:kernel_size
            ...
            next_gain_index = current_gain_index + (gain - (kernel_size + 1)/2);
            next_angle = conversionClass.wrap22pi(target_phase_states(Current_Calibration_Phase_Index) + RTPS_phase_resolution * (angle - (kernel_size + 1)/2));
            
            if (next_gain_index >= 1) && (next_gain_index <= num_achievable_gain_states) && (next_angle >= 0) && (next_angle <= 2*pi)
                k = k + 1;
            end
        end
    end
    ...
    for gain = 1:1:kernel_size
        for angle = 1:1:kernel_size
            ...
            if (next_gain_index >= 1) && (next_gain_index <= num_achievable_gain_states) && (next_angle >= 0) && (next_angle <= 2*pi)
                ...
                next_polar(k, 1) = achievable_gain_states(next_gain_index);
...
global Current_Calibration_Gain_Index Current_Calibration_Phase_Index kernel_size RTPS_phase_resolution RTPS_gain_resolution_dB target_phase_states lowest_detectable_gain_dB target_gain_states...
    RTPS_gain_resolution lowest_detectable_gain achievable_gain_states
    ...
    num_achievable_gain_states = size(achievable_gain_states, 2);
    gain_distance = abs(achievable_gain_states(1, :) - new_target_gain);
    current_gain_index = find(gain_distance == min(gain_distance));
    ...
    for gain = 1:1:kernel_size
        for angle = 1:1:kernel_size
            ...
            next_gain_index = current_gain_index + (gain - (kernel_size + 1)/2);
            next_angle = conversionClass.wrap22pi(new_target_phase + RTPS_phase_resolution * (angle - (kernel_size + 1)/2));
