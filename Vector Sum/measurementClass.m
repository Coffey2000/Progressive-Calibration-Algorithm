    ...
    methods (Static)
        ...
        global Measurements measurement_counter total_measurement_counter PS_MODEL max_gain num_actual_vector_states rfvm_40GHz_table reference_point
            ...
            elseif choice == "traditional"
                ...
                distance = abs(rfvm_40GHz_table - target_point);
                [i, j] = find(distance == min(distance, [], "all"));
                reading = rfvm_40GHz_table(i, j);
                return

            elseif choice == "polar"
                uncompensated_point = conversionClass.polar2cartesian(next(1), next(2));
                target_point = uncompensated_point - reference_point;

                [L1, L2] = conversionClass.cartesian2vectors(target_point);

                [vector1_index, vector2_index] = conversionClass.vectorLength2index(L1, L2);
...
% 
            ...
            vector1_index = conversionClass.cap(vector1_index, 1, num_actual_vector_states);
            vector2_index = conversionClass.cap(vector2_index, 1, num_actual_vector_states);

            previous_measurement = Measurements(vector1_index, vector2_index);
            
            if previous_measurement == 1234
                reading = rfvm_40GHz_table(vector1_index, vector2_index);
                Measurements(vector1_index, vector2_index) = reading;
        ...
        points = zeros(num_next, 1);
            ...
            plot(points + 0.00001*1i, "O", "LineWidth", 1.5, "MarkerSize", 10, "Color", [0 0.4470 0.7410]);
