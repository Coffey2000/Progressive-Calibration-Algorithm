global num_target_gain_states num_target_phase_states Measurements Mapping Current_Calibration_Gain_Index Current_Calibration_Phase_Index target_gain_states ...
    ...
    measurement_counter num_actual_vector_states num_actual_phase_states gain_profile max_gain_measurement max_target_gain ...
     ...
     vector1_envelop_profile vector2_envelop_profile phase_error_sum gain_error_sum VALIDATION num_hit total_num_hit LEARNING_SAMPLE_SIZE VECTOR_PROFILE_SIZE ENABLE_OUTLINE_SAMPLING USE_MACHINE_LEARNING total_RMS_gain_error_dB total_RMS_phase_error_degree


RMS_errors_vs_SAMPLE_SIZE = zeros(60, 3);

for iterations = 1:1:60

    LEARNING_SAMPLE_SIZE = iterations*50 + 50;

    disp(" ");
    disp("Current sample size: " + LEARNING_SAMPLE_SIZE);

    RMS_errors_vs_SAMPLE_SIZE(iterations, 1) = LEARNING_SAMPLE_SIZE;
    ...
    figure;
    set(gcf, 'Position',  [2000, 100, 1400, 1200]);
    %movegui(gcf,'center');
    ...
    RMS_errors_vs_SAMPLE_SIZE(iterations, 2) = total_RMS_gain_error_dB;
    RMS_errors_vs_SAMPLE_SIZE(iterations, 3) = total_RMS_phase_error_degree;

    close all

    figure
    set(gcf, 'Position',  [200, 750, 1000, 500]);
    plot(RMS_errors_vs_SAMPLE_SIZE(1:iterations, 1), RMS_errors_vs_SAMPLE_SIZE(1:iterations, 2), "LineWidth", 2);
    title("RMS Gain Error vs Sample Size");
    xlabel("Sample Size");
    ylabel("RMS Gain Error (dB)");
    
    figure
    set(gcf, 'Position',  [200, 150, 1000, 500]);
    plot(RMS_errors_vs_SAMPLE_SIZE(1:iterations, 1), RMS_errors_vs_SAMPLE_SIZE(1:iterations, 3), "LineWidth", 2);
    title("RMS Phase Error vs Sample Size");
    xlabel("Sample Size");
