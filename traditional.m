...
while Current_Calibration_Gain_Index <= Ending_Gain_Index
    for i = 1:1:num_target_phase_states
        ...
        distance = abs(simulation_data*magnitude_scaling_factor - conversionClass.polar2cartesian(target_gain_states(Current_Calibration_Gain_Index), target_phase_states(i)));
        index = find(distance == min(distance));
        Selected_Measurements(Current_Calibration_Gain_Index, i) = simulation_data(index, 1)*magnitude_scaling_factor;
    end
...
plot(simulation_data*magnitude_scaling_factor, "X")
hold on
